# Default MTK_COMBO_CHIP and hif for ARM64 builds
...
# Compilation rules (ARM64 specific)
all:
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) ARCH=arm64 modules

clean:
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) ARCH=arm64 clean
	# Remove object files, module files, and other intermediate files
	find . -name '*.ko' -exec rm {} \;
	find . -name '*.o' -exec rm {} \;
	find . -name '*.o.d' -exec rm {} \;
	find . -name '*.cmd' -exec rm {} \;
	find . -name '*odule*' -exec rm {} \;
	rm -f *.mod*

install:
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) ARCH=arm64 modules_install
...
CONFIG_MTK_DBDC_SW_FOR_P2P_LISTEN=y
#PREALLOC MEMORY
